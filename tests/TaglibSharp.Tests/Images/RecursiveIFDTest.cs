	...
	public class RecursiveIFDTestInvariantValidator : IMetadataInvariantValidator
	{
		public void ValidateMetadataInvariants (TagLib.Image.File file)
		{
			...
			// Image.0x010F (Make/Ascii/16) "Hewlett-Packard"
			{
				...
				ClassicAssert.AreEqual ("Hewlett-Packard", (entry as StringIFDEntry).Value);
			}
			// Image.0x0110 (Model/Ascii/28) "HP PhotoSmart C812 (V09.26)"
			{
				...
				ClassicAssert.IsNotNull (entry as StringIFDEntry, "Entry is not a string!");
				ClassicAssert.AreEqual ("HP PhotoSmart C812 (V09.26)", (entry as StringIFDEntry).Value);
			}
			...
			// Photo.0x829A (ExposureTime/Rational/1) "1666/100000"
			{
				...
				ClassicAssert.IsNotNull (entry as RationalIFDEntry, "Entry is not a rational!");
				ClassicAssert.AreEqual (1666, (entry as RationalIFDEntry).Value.Numerator);
				ClassicAssert.AreEqual (100000, (entry as RationalIFDEntry).Value.Denominator);
			}
			// Photo.0x829D (FNumber/Rational/1) "260/100"
			{
				...
				ClassicAssert.IsNotNull (entry as RationalIFDEntry, "Entry is not a rational!");
				ClassicAssert.AreEqual (260, (entry as RationalIFDEntry).Value.Numerator);
			...
			// Photo.0x9003 (DateTimeOriginal/Ascii/20) "2002:01:02 05:00:30"
			{
				...
				ClassicAssert.IsNotNull (entry as StringIFDEntry, "Entry is not a string!");
				ClassicAssert.AreEqual ("2002:01:02 05:00:30", (entry as StringIFDEntry).Value);
			}
			...
			// Photo.0x9203 (BrightnessValue/SRational/1) "-567/100"
			{
				...
				ClassicAssert.AreEqual (-567, (entry as SRationalIFDEntry).Value.Numerator);
			...
			// Photo.0x9206 (SubjectDistance/Rational/1) "11691/1000"
			{
				...
				ClassicAssert.IsNotNull (entry, "Entry 0x9206 missing in IFD 0");
				ClassicAssert.IsNotNull (entry as RationalIFDEntry, "Entry is not a rational!");
				ClassicAssert.AreEqual (11691, (entry as RationalIFDEntry).Value.Numerator);
			...
			// Photo.0x920A (FocalLength/Rational/1) "760/100"
			{
				...
				ClassicAssert.IsNotNull (entry as RationalIFDEntry, "Entry is not a rational!");
				ClassicAssert.AreEqual (760, (entry as RationalIFDEntry).Value.Numerator);
				ClassicAssert.AreEqual (100, (entry as RationalIFDEntry).Value.Denominator);
			}
			...
			// Photo.0xA002 (PixelXDimension/Long/1) "2272"
			{
				...
				ClassicAssert.AreEqual (2272, (entry as LongIFDEntry).Value);
			}
			// Photo.0xA003 (PixelYDimension/Long/1) "1712"
			{
				...
				ClassicAssert.IsNotNull (entry as LongIFDEntry, "Entry is not a long!");
				ClassicAssert.AreEqual (1712, (entry as LongIFDEntry).Value);
			}
			...
			// Photo.0xA20B (FlashEnergy/Rational/1) "1000/1"
			{
				...
				ClassicAssert.IsNotNull (entry, "Entry 0xA20B missing in IFD 0");
				ClassicAssert.IsNotNull (entry as RationalIFDEntry, "Entry is not a rational!");
				ClassicAssert.AreEqual (1000, (entry as RationalIFDEntry).Value.Numerator);
				ClassicAssert.AreEqual (1, (entry as RationalIFDEntry).Value.Denominator);
			}
			// Image.0x92D6 (0x92d6/Undefined/0) ""
			{
				...
				ClassicAssert.IsNotNull (entry, "Entry 0x92D6 missing in IFD 0");
				ClassicAssert.IsNotNull (entry as UndefinedIFDEntry, "Entry is not an undefined IFD entry!");
				var parsed_bytes = (entry as UndefinedIFDEntry).Data.Data;
				var bytes = new byte[] { };
				ClassicAssert.AreEqual (bytes, parsed_bytes);
			...
			// Thumbnail.0x0202 (JPEGInterchangeFormatLength/Long/1) "7591"
			{
				...
				ClassicAssert.IsNotNull (entry as LongIFDEntry, "Entry is not a long!");
				ClassicAssert.AreEqual (7591, (entry as LongIFDEntry).Value);
			}
